...
def _encode_record(record: dict):
    ...
    for idx, feature in enumerate(NUMERIC):
        ...
        scaled = (value - SCALER_MEAN[idx]) / (SCALER_SCALE[idx] if SCALER_SCALE[idx] else 1.0)
...
def _predict_one(record: dict):
    ...
    if probability >= 0.25:
        ...
        band = 'Low risk'
    # top contributors by absolute contribution
    contribs = []
    for feature, coef, value in zip(FEATURES, COEF, x):
        contribs.append({'feature': feature, 'contribution': coef * value})
    contribs = sorted(contribs, key=lambda x: abs(x['contribution']), reverse=True)[:5]
    return {
...
def metadata():
    ...
    for feature in NUMERIC:
        features.append({
            ...
            'placeholder': f'Enter a value for {feature}'
        })
    for feature in CATEGORICAL:
        features.append({
            'name': feature,
